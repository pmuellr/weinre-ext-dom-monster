...
/*
 * Hacked a bit on 2011/05/06 to convert to a weinre extension
 * Copyright (c) 2011 Patrick Mueller
 * This code is licensed under the terms of the MIT LICENSE
 */

function domMonsterAnalyze() {
...
//  setTimeout(function(){
    ...
    node.style.cssText =
      JR.reset+'text-align:left;z-index:1000000;letter-spacing:0;position:fixed;'+
      'color:#444;font:12px/13px \'Helvetica Neue\', Verdana, Arial, sans serif;'+
      'width:80%;left:10%';
    node.innerHTML =
      '<div id="jr_results_tips" style="'+JR.reset+'max-height:400px;margin:10px;padding:5px;overflow:auto;background:#fff;border:2px solid #b42328;-moz-border-radius:9px;-webkit-border-radius:9px;-webkit-box-shadow: 0px 2px 40px rgba(0,0,0,0.5);-webkit-transition:-webkit-transform ease-out 0.3s;-webkit-transform:translateY(0px)">' +
        '<div style="'+JR.reset+'height:23px;font-size:16px;font-weight:normal;margin-top:0px;margin-bottom:5px;color:#444">'+
          ...
          '</div>'+
          '<div style="'+JR.reset+'cursor:pointer;float:right;padding:5px 10px 3px 10px;height:15px;background:#b42328;-webkit-border-radius:5px;color:#fff;text-shadow:0px 1px 3px rgba(0,0,0,0.5)"><a style="color:white" href="http://mir.aculo.us/dom-monster/" target="_new">'+
            'dom monster <span style="'+JR.reset+'font-size:10px">v'+JR.Version+'</a></span>'+
          '</div>'+
